...
export default function DJConsole() {
  ...
  const nextSongMutation = useMutation({
    mutationFn: () => fetch('/api/player/next', { method: 'POST' }).then(res => res.json()),
    onSuccess: () => {
  ...
  const playPauseMutation = useMutation({
    mutationFn: (action: 'play' | 'pause') => 
      fetch(`/api/player/${action}`, { method: 'POST' }).then(res => res.json()),
    onSuccess: (_, action) => {
      setIsPlaying(action === 'play');
      toast({ title: action === 'play' ? "Playback started" : "Playback paused" });
    }
  });
  ...
  const approveSongMutation = useMutation({
    mutationFn: (songId: string) => 
      fetch(`/api/queue/${songId}/approve`, { method: 'POST' }).then(res => res.json()),
    onSuccess: (data) => {
      toast({ 
        title: "Song approved", 
  ...
  const rejectSongMutation = useMutation({
    mutationFn: (songId: string) => 
      fetch(`/api/queue/${songId}/reject`, { method: 'POST' }).then(res => res.json()),
    onSuccess: (data) => {
      toast({ 
        title: "Song rejected", 
  ...
  const playNextMutation = useMutation({
    mutationFn: (songId: string) => 
      fetch(`/api/queue/${songId}/play-next`, { method: 'POST' }).then(res => res.json()),
    onSuccess: (data) => {
      toast({ 
        title: "Now playing", 
  ...
  useEffect(() => {
    ...
    // Notifications
    newSocket.on('notification', (data: { type: string, message: string, song?: Song }) => {
      toast({ 
        title: "New Request", 
      ...
      <div className="bg-gradient-to-r from-blue-600 to-blue-700 text-white py-3 overflow-hidden relative">
        <div className="animate-ticker whitespace-nowrap">
          <span className="text-lg font-medium px-8">
            ...
            <strong className="bg-white/20 px-3 py-1 rounded-md font-mono cursor-pointer hover:bg-white/30 transition-colors mx-2"
            ...
            <Card className="sticky top-8">
              <CardContent className="p-6">
                <div className="text-center mb-6">
                  <h2 className="text-2xl font-bold text-slate-900 mb-2">Now Playing</h2>
                    ...
                    <div className="flex items-center space-x-4">
                      <div className="w-16 h-16 bg-gradient-to-br from-blue-600 to-purple-600 rounded-lg flex items-center justify-center text-white text-2xl">
                        {currentSong.thumbnail ? (
                          <img src={currentSong.thumbnail} alt="Album art" className="w-16 h-16 rounded-lg object-cover" />
                        ) : (
                    ...
                    <div className="mt-4">
                      ...
                      <div className="w-full bg-slate-200 rounded-full h-2">
                        <div className="bg-blue-600 h-2 rounded-full transition-all duration-300" 
                             style={{ width: `${position / 180 * 100}%` }}></div>
                      </div>
                    </div>
                  )}
                </div>
                ...
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="flex items-center space-x-4">
                    ...
                    <Button 
                      ...
                      onClick={() => queryClient.invalidateQueries({ queryKey: ['/api/queue'] })}
                    >
                      Refresh
                    </Button>
                  </div>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full">
                    <thead className="bg-slate-50 border-b border-slate-200">
                      <tr>
                        <th className="text-left p-4 font-semibold text-slate-700">Song</th>
                        <th className="text-left p-4 font-semibold text-slate-700">Requester</th>
                        <th className="text-center p-4 font-semibold text-slate-700">Duration</th>
                      ...
                      {queue.length === 0 ? (
                        <tr>
                          <td colSpan={5} className="text-center py-12">
                            ...
                            <h3 className="text-lg font-semibold text-slate-900 mb-2">No songs in queue</h3>
                            <p className="text-slate-600 mb-4">Share the email address above to start receiving song requests!</p>
                          </td>
                        </tr>
                      ) : (
                        queue.map((song) => (
                            ...
                            <td className="p-4">
                              <div className="flex items-center space-x-3">
                                <div className="w-12 h-12 bg-gradient-to-br from-red-500 to-pink-600 rounded-lg flex items-center justify-center text-white">
                                  {song.thumbnail ? (
                                    <img src={song.thumbnail} alt="Song thumbnail" className="w-12 h-12 rounded-lg object-cover" />
                                  ) : (
